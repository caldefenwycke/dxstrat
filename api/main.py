...
DB_PATH='db/pool.db'; CFG_PATH='config/config.json'
...
def require_admin(t):
    if not t or t!=cfg()['security']['admin_token']: raise HTTPException(401,'unauthorized')
...
def stats():
    with db() as c:
        m=c.execute('SELECT COUNT(*) as c FROM miners').fetchone()['c']
        s=c.execute('SELECT COUNT(*) as c FROM shares').fetchone()['c']
        b=c.execute('SELECT COUNT(*) as c FROM blocks').fetchone()['c']
        bal=c.execute('SELECT COALESCE(SUM(balance_sats),0) as t FROM miners').fetchone()['t']
    return {'miners':m,'shares':s,'blocks':b,'total_balance_sats':bal}

@app.get('/miners')
def miners():
    with db() as c:
        rows=c.execute('SELECT id,address,last_seen_ts,difficulty,total_accepted,total_rejected,balance_sats FROM miners ORDER BY last_seen_ts DESC LIMIT 500').fetchall()
    return {'miners':[dict(r) for r in rows]}

# ---- Public Miner Dashboard ----
with open('api/miner.html','r',encoding='utf-8') as f:
    MINER_HTML = f.read()
...
def miner_stats(address: str):
    ...
    with db() as c:
        ...
        # difficulty-1 shares per second -> hashrate ~ sum(diff) * 2^32 / window_secs
...
def miner_shares(address: str, limit: int = 100):
    ...
    with db() as c:
        ...
        if not m:
            return {'items': []}
        rows = c.execute('SELECT received_ts, valid, difficulty, pow_hash FROM shares WHERE miner_id=? ORDER BY received_ts DESC LIMIT ?', (m['id'], limit)).fetchall()
...
def miner_hashrate_24h(address: str, bucket_minutes: int = 5):
    ...
    # bucketize
    n_buckets = math.ceil((24*3600)/bucket_secs)
    buckets = [0.0]*n_buckets
    t0 = start - (start % bucket_secs)
    for r in rows:
        idx = (r['received_ts'] - t0)//bucket_secs
        if 0 <= idx < n_buckets:
            buckets[int(idx)] += float(r['difficulty'])
    labels = [t0 + i*bucket_secs for i in range(n_buckets)]
    # hashrate per bucket: sum(diff)*2^32 / bucket_secs
...
# ---- Darwin X live stats ----
with open('api/darwinx.html','r',encoding='utf-8') as f:
    DARWINX_HTML = f.read()
...
def darwinx_stats():
    with db() as c:
        row = c.execute('SELECT template_prevhash, template_height, started_ts, pool_size_current, generated_since_template, top_score, top_hash_norm, top_entropy, w_hash, w_ent, last_update, top_list FROM engine_stats WHERE id=1').fetchone()
        if not row:
            return {'ok': False}
        d = dict(row)
        import json as _json
        try:
            d['top_list'] = _json.loads(d.get('top_list') or '[]')
        except Exception:
            d['top_list'] = []
        return {'ok': True, **d}
